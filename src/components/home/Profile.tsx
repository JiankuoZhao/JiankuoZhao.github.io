...
import { Github, Linkedin, MessageCircle, Pin } from 'lucide-react';
...
export default function Profile({ author, social, features, researchInterests }: ProfileProps) {
    ...
    useEffect(() => {
        ...
        const userHasLiked = localStorage.getItem('jiankuozhao-website-user-liked');
    ...
    const handleLike = () => {
        ...
        if (newLikedState) {
            localStorage.setItem('jiankuozhao-website-user-liked', 'true');
            setShowThanks(true);
            setTimeout(() => setShowThanks(false), 2000);
        } else {
            localStorage.removeItem('jiankuozhao-website-user-liked');
            setShowThanks(false);
        }
    };
    ...
    const locationDetails = social.location_details || [];

    const socialLinks = [
        ...(allEmails.length > 0 ? [{
            name: 'Email',
            href: `mailto:${allEmails[0]}`,
            icon: EnvelopeIcon,
            isEmail: true,
        }] : []),
        ...(social.location || locationDetails.length > 0 ? [{
            name: 'Location',
                                        ...
                                        >
                                            <div className="text-center">
                                                ...
                                                <div className="space-y-1">
                                                    {allEmails.map((email) => (
                                                        <p key={email} className="break-words">{email.replace('@', ' (at) ')}</p>
                                                    ))}
                                                </div>
                                                <div className="mt-2">
                                                    <a
                                    ...
                                    {(showWechat || isWechatPinned) && (
                                        ...
                                        >
                                            <div className="text-center">
                                                ...
                                                <p className="break-words">{social.wechat}</p>
                                            </div>
                                            <div className="absolute top-full left-1/2 transform -translate-x-1/2 w-0 h-0 border-l-4 border-r-4 border-t-4 border-transparent border-t-neutral-800"></div>
                                        </motion.div>
                                    )}
